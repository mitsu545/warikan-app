...
| 段階 | やること | 確認方法 |
|---|---|---|
| **1** ✅ | スプレッドシート7シートを用意。GAS で `save` / `summary` / `receipt` を通す。アプリの設定画面から接続 | 手入力で1件保存し、相手のスマホに出るか（手順は `setup.md`） |
| **2** ✅ | 写真：ドライブ保存、`photo` で取得、日次の自動削除トリガー | 2人の端末で同じ写真が見えるか |
| **3** ✅ | Gemini の `ocr`（店名・日付・合計・品目を読む）。キーは `GEMINI_KEY` に置く。**実物のレシート5枚以上**で精度確認 | 明細なしレシートも1枚は試す |
